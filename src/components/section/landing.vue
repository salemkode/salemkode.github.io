<template>
  <section class="section-landing container">
    ...
    <div class="content">
      <div class="info">
        <h1>
          ...
          <img
            ...
            class="hand-icon"
            :class="{ stop: stopAnimation }"
          />
        </h1>
        <h1>
          I'm
          <span class="w-3" />
          <strong>Salem Shamakh</strong>
        </h1>
      ...
      <div class="photo">
        <img class="avater" src="landing/avater.png" alt="avater" />
      </div>
    </div>
    <div @click="down" class="mouse">
      <svg
        width="30"
        height="45"
        viewBox="0 0 30 45"
...
export default {
  mounted() {
    ...
    setTimeout(() => (vue.stopAnimation = true), 1000)
  },
  data() {
    return {
      stopAnimation: false,
  ...
  methods: {
    down() {
      let elmScroll =
        document.querySelector('.section-landing').offsetHeight -
...
.section-landing h1 .hand-icon.stop {
  animation: none;
}
.section-landing h1 .hand-icon.stop:hover {
  animation: 1s hello;
  animation-direction: alternate;
}
...
.section-landing .mouse:hover rect {
